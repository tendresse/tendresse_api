...
from ..schemas.achievement import achievement_schema, achievements_schema, \
    achievements_without_tags_schema
from ..schemas.blog import blog_schema, blogs_schema, \
    blogs_without_gifs_schema
from ..schemas.gif import gif_schema, gifs_schema
from ..schemas.tag import tag_schema, tags_schema, tags_without_gifs_schema
...
def ready():
    ...
    pending_tendresses = [
        t for t in current_user.tendresses_received if not t.state_viewed]
...
def send_tendresse(username_friend):
    ...
    if friend is not None:
        ...
        if ach is not []:
            for a in ach:
                emit('new achievement', success_schema.jsonify(
                    a), room=friend.username)
...
def add_blog(blog):
    ...
    blog_json_url = "https://api.tumblr.com/v2/blog/" + \
        scheme.data.url + "/posts/photo?api_key=" + tumblr_key
            ...
            for post in data["response"]["posts"]:
                if post["type"] == "photo":
                    for photo in post["photos"]:
                        ...
                        if url_gif[-3:] == "gif":
                            # photo["original_size"]["width"] pour trier si le
                            # gif est trop petit ?
                            gif = Gif(url=url_gif)
                            db.session.add(gif)
                            for l_tag in post["tags"]:
                                # sometimes l_tag is a list of tag seperated by blank spaces
                                # so we need to split this string ang get a
                                # list of tags
                                l_tag = l_tag.split(' ')
                                for tag in l_tag:
                                    tag = tag.lower()
